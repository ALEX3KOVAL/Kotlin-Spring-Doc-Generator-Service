Реактивный сервис электронного документооборота (генерация документов по заданным docx-шаблонам с якорями-заглушками для переменных dto (модели) документа, которые загружаются при обращении к [file-service](https://github.com/ALEX3KOVAL/Java-Spring-File-Service)).
Для конвертации файлов в pdf используется [сервис с запущенным NodeJS сервером, API для конвертации и установленным Unoconv](https://github.com/zrrrzzt/tfk-api-unoconv.git).
